...
schema.constructors.forEach((item) => {
  ...
  if (itemName.split('.').length == 2) {
    ...
    if (schemaNamespace[nameSpace] == undefined) schemaNamespace[nameSpace] = [item];
    else schemaNamespace[nameSpace].push(item);
    return;
  }
...
Object.keys(schemaNamespace).forEach((nameSpace) => {
  const items: Item[] = schemaNamespace[nameSpace];
...
// write output
writeFileSync(resolve(__dirname, '../TL/Schema.ts'), lines.join('\n\n'));
