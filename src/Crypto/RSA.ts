import bigInt from 'big-integer';

const rsaPublicKey = `-----BEGIN RSA PUBLIC KEY-----

Slv8kg9qv1m6XHVQY3PnEw+QQtqSIXklHwIDAQAB
-----END RSA PUBLIC KEY-----`;

// const rsaPublicKey = ``;

const publicKey1 = `-----BEGIN PUBLIC KEY-----

JwIDAQAB
-----END PUBLIC KEY-----`;

const publicKey2 = `-----BEGIN PUBLIC KEY-----

/wIDAQAB
-----END PUBLIC KEY-----`;

const publicKey3 = `-----BEGIN PUBLIC KEY-----

oQIDAQAB
-----END PUBLIC KEY-----`;

const publicKey4 = `-----BEGIN PUBLIC KEY-----

AQIDAQAB
-----END PUBLIC KEY-----`;

export const publicServerKeys = new Map([
  // c3b42b026ce86b21
  [
    // '14101943622620965665', //unsigned
    '-4344800451088585951',
    {
      modulus:
        '',
      exponent: '010001'
    }
  ],
  // 10bc35f3509f7b7a5
  [
    '19294369909990471589',
    {
      modulus:
        '',
      exponent: '010001'
    }
  ],
  // 115ae5fa8b5529542
  [
    '20009035372654925122',
    {
      modulus:
        '',
      exponent: '010001'
    }
  ],
  // aeae98e13cd7f94f
  [
    '12587166101702965583',
    {
      modulus:
        '',
      exponent: '010001'
    }
  ],
  // 15a181b2235057d98
  [
    '24938712770296511896',
    {
      modulus:
        '',
      exponent: '010001'
    }
  ]
]);

// const a = {1:{'a': 1}}

export function encrypt(data: Buffer, fingerprint: bigint) {
  // const dataInt = new BigNumber(data.toString('hex'), 16);
  const dataInt = bigInt(data.toString('hex'), 16);

  const publicKey = publicServerKeys.get(fingerprint.toString());
  // const expo = new BigNumber(publicKey.exponent, 16);
  // @ts-ignore
  const expo = bigInt(publicKey.exponent, 16);
  // const modu = new BigNumber(publicKey.modulus, 16);
  // @ts-ignore
  const modu = bigInt(publicKey.modulus, 16);
  // console.log(expo, modu.toString(10))
  // return dataInt.pow(expo).mod(modu);
  return Buffer.from(dataInt.modPow(expo, modu).toString(16), 'hex');
}
